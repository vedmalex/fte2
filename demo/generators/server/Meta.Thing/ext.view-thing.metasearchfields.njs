...
Ext.define('Grainjs.metasearchfields.#{context.$namespace}.#{context.$name}', {
  override: 'Grainjs.metadata',
  statics:{
    'searchfields.#{context.$namespace}.#{context.$name}': {
    <#
      for( let i = 0; i< properties.length; i +=1){
        const f = properties[i]
          ...
          // тут может быть много полей и все они удалятся
              #>
            [`#{property.propertyName}::#{f.displayName}`]: (legacy)=>{
              const res = {
                name:               '#{property.propertyName}',
                format:             #{getFormat(f)},
                plugins: <#if(needClear(f, true)){#>[`clearbutton`]<#}else{#>[]<#}#>,
              ...
              <#if (f.prepareForSearch !== undefined){#>
								prepareFn: function (value) {
									#{f.prepareForSearch}
								},<#}#>
                dataType: '#{property.type.toLowerCase()}',
                xtype: '#{searchTypes[f.fieldtype]}',
                <#if(f.displayName !== '_'){#>
                cls:   "custom-x-field",
                    ...
                    items:[
                      {
                        ...
                        xtype: 'checkbox',
                        boxLabel  : 'exists',
                        name: '#{property.propertyName}|exists',
                        margin: "0 5 5",
                      },
                      {
                        extraSearchOption:true,
                        optionName:'existsValue',
                        propertyName: '#{property.propertyName}',
                        flex: 0.3,
                        xtype: 'radiogroup',
                        layout: 'hbox',
                        items:[
                          {
                            ...
                            margin: "0 5 5",
                            boxLabel: 'Y',
                            name: '#{property.propertyName}|exists_yn',
                            inputValue: true,
                            checked : true
                          },
                          {
                      ...
                      },
                      {
                        ...
                        optionName: "json",
                        propertyName: '#{property.propertyName}',
                        name: '#{property.propertyName}|json',
                        xtype: 'textfield',
                        labelWidth: 0,
                      }
                    ]
