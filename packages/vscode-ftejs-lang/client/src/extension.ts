...
export async function activate(context: vscode.ExtensionContext) {
  ...
  const clientOptions: LanguageClientOptions = {
    ...
    synchronize: {
      ...
      fileEvents: vscode.workspace.createFileSystemWatcher(
        `**/*.{${sharedExt.templateExtensionsAll.map((e: string) => e.slice(1)).join(',')}}`,
      ),
    },
  }
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('ftejs.scaffold.block', () =>
      insert(`<# block '\${1:name}' : #>\n$0\n<# end #>`),
    ),
    vscode.commands.registerCommand('ftejs.scaffold.slot', () =>
      insert(`<# slot '\${1:name}' : #>\n$0\n<# end #>`),
    ),
    vscode.commands.registerCommand('ftejs.scaffold.chunkPair', () =>
      insert(`<#- chunkStart('\${1:path}'); -#>\n$0\n<# chunkEnd(); -#>`),
    ),
    vscode.commands.registerCommand('ftejs.scaffold.partial', async () => {
      const editor = vscode.window.activeTextEditor
      if (!editor) return
      const name = await vscode.window.showInputBox({
        prompt: 'Partial name (file base name without extension)',
        value: 'partial',
      })
      if (!name) return
      ...
      try {
        ...
        editor.insertSnippet(
          new vscode.SnippetString(`#{partial(context, '${name}')}\n`),
        )
        const open = await vscode.window.showQuickPick(
          ['Open partial', 'Skip'],
          { placeHolder: 'Open created partial?' },
        )
    ...
    vscode.commands.registerCommand(
      ...
      async () => {
        ...
        try {
          const lang =
            editor.document.languageId === 'template-html'
              ? 'html'
              : editor.document.languageId === 'template-markdown'
                ? 'markdown'
                : editor.document.languageId === 'template-typescript'
                  ? 'typescript'
                  : 'javascript'
          const views = await client.sendRequest<any>('ftejs/extractViews', {
            uri,
            hostLanguage: lang,
          })
          const panel = vscode.window.createWebviewPanel(
            'ftejsDualPreview',
            'fte.js Dual Extraction Preview',
    ...
    // Format current document via server's formatter
    vscode.commands.registerCommand('ftejs.formatDocument', async () => {
      const editor = vscode.window.activeTextEditor
      if (!editor) return
      try {
        const edits = await vscode.commands.executeCommand<vscode.TextEdit[]>(
          ...
          editor.document.uri,
          { insertSpaces: true, tabSize: editor.options.tabSize || 2 },
        )
        if (edits && edits.length) {
          ...
          for (const e of edits)
            ws.replace(
              editor.document.uri,
              e.range,
              editor.document.getText(e.range).length ? e.newText : e.newText,
            )
    ...
    vscode.commands.registerCommand(
      ...
      async (payload: {
        ...
        const selectionText = editor.document.getText(
          new vscode.Range(payload.range.start, payload.range.end),
        )
        const blockDecl = `<# block '${name}' : #>\n${selectionText}\n<# end #>\n`
        const ws = new vscode.WorkspaceEdit()
        ws.insert(
          editor.document.uri,
          new vscode.Position(payload.range.start.line, 0),
          blockDecl,
        )
        ws.replace(
          editor.document.uri,
    ...
    vscode.commands.registerCommand('ftejs.generator.ntsClass', async () => {
      ...
      const fields = await vscode.window.showInputBox({
        prompt: 'Fields (comma separated name:type)',
    ...
    vscode.commands.registerCommand('ftejs.preview.chunks', async () => {
      ...
      const starts = [
        ...txt.matchAll(/<#-\s*chunkStart\(\s*(["'`])([^"'`]+)\1\s*\);\s*-#>/g),
      ]
      ...
      for (let i = 0; i < starts.length; i++) {
        ...
        const next =
          i + 1 < starts.length
            ? (starts[i + 1].index ?? txt.length)
            : txt.length
        const endIdx =
          txt.indexOf('<# chunkEnd(); -#>', from) >= 0
            ? txt.indexOf('<# chunkEnd(); -#>', from)
            : next
        chunks.push({ name, start: from, end: endIdx >= 0 ? endIdx : next })
      }
      const panel = vscode.window.createWebviewPanel(
        'ftejsPreview',
        'fte.js Chunks Preview',
        vscode.ViewColumn.Beside,
        { enableScripts: false },
      )
      const html = ['<h3>Detected chunks (static)</h3>']
        .concat(
          chunks.map(
            (c) =>
              `<h4>${c.name}</h4><pre>${escapeHtml(txt.slice(c.start, c.end))}</pre>`,
          ),
        )
        .join('\n')
    ...
    vscode.commands.registerCommand('ftejs.preview.chunksLive', async () => {
      ...
      try {
        ...
        const factory = new Factory({
          root: [path.dirname(fsPath)],
          ext: sharedExt.templateExtensionsFactory,
          watch: false,
          preload: true,
        })
        const name = path.basename(fsPath)
        const res = await factory.run(
          { title: 'Preview', items: ['A', 'B'] },
          name,
        )
        const panel = vscode.window.createWebviewPanel(
          'ftejsPreviewLive',
          'fte.js Chunks Preview (Live)',
          vscode.ViewColumn.Beside,
          { enableScripts: false },
        )
        ...
        if (Array.isArray(res)) {
          html += res
            .map(
              (c: any) =>
                `<h4>${escapeHtml(c.name)}</h4><pre>${escapeHtml(Array.isArray(c.content) ? c.content.join('\n') : String(c.content))}</pre>`,
            )
            .join('')
        } else if (typeof res === 'object') {
          html += Object.keys(res)
            .map(
              (k: string) =>
                `<h4>${escapeHtml(k)}</h4><pre>${escapeHtml(String((res as any)[k]))}</pre>`,
            )
            .join('')
        } else {
    ...
    vscode.commands.registerCommand('ftejs.convertToTemplate', async () => {
      ...
      if (!dstExt) {
        vscode.window.showWarningMessage(
          `Unsupported source type '${ext}' for template conversion. Supported: ${Object.keys(map).join(', ')}`,
        )
        return
      }
      ...
      try {
        ...
        const exists = await vscode.workspace.fs.stat(dstUri).then(
          () => true,
          () => false,
        )
        if (exists) {
  ...
  context.subscriptions.push(
    vscode.window.onDidChangeActiveTextEditor((ed) =>
      scheduleUpdateTrimDecorations(ed),
    ),
  ...
  // The LSP server now delegates formatting to the shared fte.js-formatter package
  // context.subscriptions.push(
