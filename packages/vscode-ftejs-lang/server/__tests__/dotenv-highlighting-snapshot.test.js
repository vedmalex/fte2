...
describe('dotenv template snapshot (adjacent #>#<# and expr)', () => {
  test('grammar patterns cover adjacent open/close and expr', () => {
    ...
    const html = JSON.parse(
      fs.readFileSync(root + '/syntaxes/template-html.tmLanguage.json', 'utf8'),
    )
    const js = JSON.parse(
      fs.readFileSync(root + '/syntaxes/template-js.tmLanguage.json', 'utf8'),
    )
    const ts = JSON.parse(
      fs.readFileSync(
        root + '/syntaxes/template-typescript.tmLanguage.json',
        'utf8',
      ),
    )
    const md = JSON.parse(
      fs.readFileSync(
        root + '/syntaxes/template-markdown.tmLanguage.json',
        'utf8',
      ),
    )
