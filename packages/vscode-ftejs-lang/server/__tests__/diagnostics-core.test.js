...
describe('Diagnostics core', () => {
  test('detects unmatched end', () => {
    ...
    expect(
      diags.some(
        (d) => /Unmatched end/.test(d.message) && d.severity === 'error',
      ),
    ).toBe(true)
  })
  ...
  test('detects duplicate blocks', () => {
    ...
    const diags = computeDiagnosticsFromText(txt)
    expect(
      diags.some((d) => /Duplicate block declaration: a/.test(d.message)),
    ).toBe(true)
  })
  ...
  test('detects blocks with unusual spacing and quotes', () => {
    ...
    const diags = computeDiagnosticsFromText(txt)
    expect(
      diags.some((d) => /Duplicate block declaration: test/.test(d.message)),
    ).toBe(true)
  })
  ...
  test('warns on unknown content name', () => {
    ...
    const diags = computeDiagnosticsFromText(txt)
    expect(
      diags.some((d) => /Unknown block name: missing/.test(d.message)),
    ).toBe(true)
  })
  ...
  test('warns on unresolved partial', () => {
    ...
    expect(
      diags.some((d) => /Unresolved partial: not\/exists/.test(d.message)),
    ).toBe(true)
  })
})
