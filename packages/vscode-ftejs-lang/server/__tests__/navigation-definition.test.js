...
describe('Navigation Definition Fix', () => {
  ...
  function resolveDefinition(doc, text, offset, ast) {
    ...
    while ((match = contentRegex.exec(around)) !== null) {
      ...
      const quoteStart =
        winStart + contentStart + match[0].indexOf(match[1]) + 1
...
<div>
  ...
  #{content('footer')}
    ...
    const contentCallPos =
      inputText.indexOf("content('content')") + "content('".length
...
<div>
  #{content('footer')}
    ...
    const footerCallPos =
      inputText.indexOf("content('footer')") + "content('".length
