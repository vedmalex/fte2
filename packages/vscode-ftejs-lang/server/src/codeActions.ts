...
export function buildCodeActions(params: {
  ...
  const { text, uri, range, diagnostics, doc, indentSize, parseContent } =
    params
  const actions: any[] = []

  const hasUnmatchedEnd = diagnostics.some((d) =>
    /Unmatched end/.test(d.message),
  )
  if (hasUnmatchedEnd) {
    actions.push({
    ...
    if (m) {
      ...
      if (local) {
        ...
        actions.push({
          ...
          edit: {
            changes: {
              [uri]: [TextEdit.replace({ start: from, end: to }, sanitized)],
            },
          },
        })
      }
  ...
  if (stack.length) {
    ...
    const tags = stack
      .map(
        (s) =>
          `<#${s.trimmedOpen ? '-' : ''} end ${s.trimmedClose ? '-' : ''}#>`,
      )
      .join(`\n${indent}`)
    actions.push({
      ...
      kind: 'quickfix',
      edit: {
        changes: { [uri]: [TextEdit.insert(range.end, `\n${indent}${tags}`)] },
      },
    })
  }
  ...
  if (selectionText && selectionText.length > 0) {
    actions.push({
      ...
      edit: {
        changes: {
          [uri]: [TextEdit.replace(range, `<#- ${selectionText} -#>`)],
        },
      },
    })
    actions.push({
      ...
      kind: 'refactor.rewrite',
      edit: {
        changes: { [uri]: [TextEdit.replace(range, `<# ${selectionText} #>`)] },
      },
    })
    actions.push({
  ...
  if (open >= 0 && closeRel >= 0) {
    ...
    if (exprText.trim().length > 20) {
      ...
      const insertDecl = TextEdit.insert(
        lineStart,
        `<# const ${varName} = ${exprText.trim()} #>\n`,
      )
      const replaceRange = Range.create(
        doc.positionAt(exprStart),
        doc.positionAt(exprEnd),
      )
