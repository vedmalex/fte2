...
export function getDefinition(
  ...
  while ((contentMatch = contentRegex.exec(around)) !== null) {
    ...
    const quoteStart =
      winStart + contentStart + contentMatch[0].indexOf(contentMatch[1]) + 1
      ...
      if ((ast as any)?.blocks?.[blockName]) {
        ...
        if (first) {
          return Location.create(
            docUri,
            Range.create(
              posFromOffset(text, first.pos),
              posFromOffset(text, first.pos + (first.content?.length || 0)),
            ),
          )
        }
      }
        ...
        try {
          ...
          if (pAst?.blocks?.[blockName]) {
            ...
            if (parentBlock.declPos !== undefined) {
              ...
              const declLength =
                (parentBlock.declStart || '').length +
                (parentBlock.declContent || '').length +
                (parentBlock.declEnd || '').length
              const declEnd = posFromOffset(
                src,
                parentBlock.declPos + declLength,
              )
            ...
            if (first) {
              ...
              return Location.create(
                uri,
                Range.create(
                  posFromOffset(src, first.pos),
                  posFromOffset(src, first.pos + (first.content?.length || 0)),
                ),
              )
            }
          }
  ...
  while ((match = openRe.exec(text))) {
    ...
    if (offset >= nameStart && offset <= nameEnd) {
        ...
        try {
          ...
          if (dm) {
            ...
            return Location.create(
              uri,
              Range.create(
                posFromOffset(src, dm.index),
                posFromOffset(src, dm.index + dm[0].length),
              ),
            )
          }
        } catch {}
      ...
      return Location.create(
        docUri,
        Range.create(
          posFromOffset(text, match.index),
          posFromOffset(text, match.index + match[0].length),
        ),
      )
    }
  }
...
export function getReferences(
  ...
  const usageRe = new RegExp(
    String.raw`content\(\s*(["'\`])${selected.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}\1`,
    'g',
  )
  const slotRe = new RegExp(
    String.raw`slot\(\s*(["'\`])${selected.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}\1`,
    'g',
  )
  ...
  while ((u = usageRe.exec(text))) {
    res.push(
      Location.create(
        docUri,
        Range.create(
          posFromOffset(text, u.index),
          posFromOffset(text, u.index + u[0].length),
        ),
      ),
    )
  }
    ...
    while ((mu = usageRe.exec(p))) {
      res.push(
        Location.create(
          uri,
          Range.create(
            posFromOffset(p, mu.index),
            posFromOffset(p, mu.index + mu[0].length),
          ),
        ),
      )
    }
...
export function getHover(
  ...
  if (!ast) return null
  const hit = (ast.main as any[])?.find(
    (n: any) => offset >= n.pos && offset <= n.pos + (n.content?.length || 0),
  )
  if (hit) {
    const around = text.slice(
      Math.max(0, offset - 40),
      Math.min(text.length, offset + 40),
    )
    const func = around.match(/\b(partial|content|slot|chunkStart|chunkEnd)\b/)
    if (func) {
      const key = func[1]
      const info =
        usageDocs.functions[key] ||
