...
export function computeOpenBlocksFromAst(
  nodes: AstNode[],
  upTo?: number,
): OpenBlock[] {
...
export function walkAstNodes(
  ast: any,
  callback: (node: any) => void | boolean,
): void {
...
export function resolveTemplateRel(
  rel: string,
  docUri: string | undefined,
...
export function computeJsCodeDelta(line: string): {
  dedentFirst: number
  delta: number
} {
...
export function collectAllASTSegments(ast: any): any[] {
  ...
  for (const [blockName, block] of Object.entries<any>(ast.blocks || {})) {
    ...
    if (blockContent.length > 0) {
      ...
      const blockEndPos =
        (lastItem.pos || 0) + (lastItem.content?.length || 0) + 10
      segmentsByPos.push({
        pos: blockEndPos,
        segment: {
          type: 'blockEnd',
...
export function extractBlockAndSlotSymbols(ast: any): {
  blocks: BlockInfo[]
  slots: SlotInfo[]
} {
...
export function validateStructureAndCollectErrors(
  ...
  unmatchedEnds: Array<{ pos: number; len: number }>
  unclosed: Array<{ name: string; pos: number }>
  parserErrors: Array<{ pos: number; message: string }>
} {
  ...
  for (const n of ast.main as any[]) {
    if (n.type === 'blockStart' || n.type === 'slotStart') {
      ...
      if (stack.length === 0) {
        const len =
          text.slice(n.pos).match(/^<#-?\s*end\s*-?#>/)?.[0]?.length || 5
...
export function collectTrimWhitespaceHints(
  text: string,
): Array<{ start: number; end: number; kind: 'left' | 'right' }> {
...
export function validateDirectivesInText(text: string): Array<{
  start: number
  end: number
  message: string
  ...
  try {
    ...
    for (const [blockName, blockInfo] of Object.entries<any>(ast.blocks)) {
      if (!parentBlocks.has(blockName)) {
        const declPos =
          (blockInfo as any).declPos ?? (blockInfo as any).pos ?? 0
