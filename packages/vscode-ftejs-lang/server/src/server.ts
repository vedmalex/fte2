...
function indexText(uri: string, text: string, absPath?: string) {
  ...
  if (prev?.extendsPath) {
    ...
    if (set) {
      set.delete(uri)
      if (set.size === 0) extendsChildren.delete(prev.extendsPath)
    }
  }
  ...
  for (const p of pathCandidates) {
    try {
      ...
      for (const line of lines) {
        ...
        const mChunks = line.match(
          /^###\s+Чанки.*chunkStart\(name\).*chunkEnd\(\)/,
        )
...
connection.onInitialize((params: InitializeParams) => {
  try {
    ...
    const wsFolders =
      params.workspaceFolders?.map((f) => url.fileURLToPath(f.uri)) || []
...
connection.onRequest(
  ...
  (params: {
    uri: string
    hostLanguage?: 'html' | 'markdown' | 'javascript' | 'typescript'
  }) => {
    try {
      ...
      const instrView = extractInstructionCodeView(text, ast, {
        hostLanguage: host,
        instructionLanguage:
          host === 'typescript' ? 'typescript' : 'javascript',
      })
...
async function refreshSettings() {
  try {
    const cfg: any = await (connection as any).workspace?.getConfiguration?.(
      'ftejs',
    )
...
connection.onDocumentFormatting(({ textDocument, options }) => {
  try {
    ...
    if (textDocument.uri !== originalUri) {
      logError(
        new Error('URI changed during formatting'),
...
connection.onDocumentOnTypeFormatting(
  ({ ch, options, position, textDocument }) => {
    ...
    const currentLineIndent =
      before.slice(currentLineStart).match(/^\s*/)?.[0]?.length ?? 0
...
documents.onDidChangeContent(({ document }) => {
  ...
  // Integrate external linter diagnostics (non-structural hygiene)
  try {
  ...
  const hints = diags.filter(
    (d) => d.severity === DiagnosticSeverity.Hint,
  ).length
  logInfo(
    `Diagnostics updated: ${errors} error(s), ${warns} warning(s), ${hints} hint(s)`,
    'diagnostics',
    { uri: document.uri },
  )
...
documents.onDidOpen(({ document }) => {
  ...
  logInfo(
    `Diagnostics on open: ${errors} error(s), ${warns} warning(s), ${hints} hint(s)`,
    'diagnostics',
    { uri: document.uri },
  )
  try {
