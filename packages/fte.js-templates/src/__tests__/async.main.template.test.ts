...
describe('MainTemplate.njs async mode', () => {
  test('awaits promise expressions and joins with __aj', async () => {
    ...
    const html = await Local.runAsync(
      { promiseName: Promise.resolve('X') } as any,
      'x.njs',
    )
  ...
  test('partials that return Promise are awaited', async () => {
    ...
    const ctx: any = {
      ...
      main: [
        {
          type: 'expression',
          content: "partial(context, 'part.njs')",
          start: true,
          end: true,
          eol: true,
        },
    ...
    const cfgPart =
      typeof corePart === 'string'
        ? eval('(' + corePart + ')')
        : eval('(' + corePart.code + ')')
    const cfgMain =
      typeof coreMain === 'string'
        ? eval('(' + coreMain + ')')
        : eval('(' + coreMain.code + ')')
  ...
  test('slots with Promise content are joined', async () => {
    ...
    const ctx: any = {
      ...
      main: [
        ...
        {
          type: 'expression',
          content: "slot('a', [Promise.resolve('L')])",
          start: true,
          end: true,
          eol: false,
        },
