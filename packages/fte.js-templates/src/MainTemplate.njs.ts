import { type TemplateBase, TemplateSourceMapGenerator } from 'fte.js-base'
...
export default {
  ...
  script: function (
    ...
    const mainResult = partial(mainBlocks, 'codeblock', partialOptions)
    const mainCode =
      typeof mainResult === 'string' ? mainResult : mainResult.code
    if (typeof mainCode !== 'string') {
      throw new Error(
        'MainTemplate.njs: codeblock returned non-string for main',
      )
    }
    ...
    if (streamMode) {
      ...
      if (directives.chunks) {
        ...
        out.push(
          '      const maxSize = options && options.maxCoalesceChunkSize || 0\n',
        )
        out.push("      let acc=''\n")
        out.push(
          "      for(const v of arr){ if(__ab && __ab.aborted) return; const sv = (__isThenable(v)? await v : v); if(maxSize>0){ acc += sv; if(acc.length>=maxSize){ yield acc; acc='' } } else { yield sv } }\n",
        )
      ...
      if (asyncMode) {
        ...
        out.push(
          "    const __aj = async arr => { const a = await Promise.all(Array.from(arr, v => __isThenable(v)? v : Promise.resolve(v))); return Array.isArray(a) ? a.join('') : String(a) }\n",
        )
      }
    }
    out.push(
    ...
    if (blockNames.length > 0) {
      ...
      for (let i = 0; i < blockNames.length; i += 1) {
        ...
        const blockResult = partial(blockMain, 'codeblock', partialOptions)
        const blockCode =
          typeof blockResult === 'string' ? blockResult : blockResult.code
        if (typeof blockCode !== 'string') {
          throw new Error(
            "MainTemplate.njs: codeblock returned non-string for block '" +
              blockNames[i] +
              "'",
          )
        }
    ...
    if (slotNames.length > 0) {
      ...
      for (let i = 0; i < slotNames.length; i += 1) {
        ...
        const slotResult = partial(slotMain, 'codeblock', partialOptions)
        const slotCode =
          typeof slotResult === 'string' ? slotResult : slotResult.code
        if (typeof slotCode !== 'string') {
          throw new Error(
            "MainTemplate.njs: codeblock returned non-string for slot '" +
              slotNames[i] +
              "'",
          )
        }
    ...
    if (mainMap && SourceMapGeneratorCtor) {
      ...
      const prefixLines =
        startIdx >= 0 ? result.slice(0, startIdx).split(/\r?\n/).length : 0
      ...
      const primarySource =
        Array.isArray((mainMap as any).sources) &&
        (mainMap as any).sources.length
