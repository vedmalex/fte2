  ...
  script: function (files, _content, partial, slot, options) {
    ...
    // TS interface for context if provided by analyzer
    {
      ...
      for (let i = 0; i < files.length; i += 1) {
        const t =
          files[i].template &&
          files[i].template.directives &&
          (files[i].template.directives as any).contextTypes
    ...
    files.forEach((file) => {
      ...
      if (typeof coreCode !== 'string') {
        throw new Error(
          'singlefile.ts.njs: core template returned invalid result for ' +
            file.name,
        )
      }
      out.push(
        "  ['" + (file.template.alias || file.name) + "']: " + coreCode + ',',
      )
    })
    out.push('\n')
    out.push('}\n')
