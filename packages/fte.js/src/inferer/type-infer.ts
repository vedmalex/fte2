...
function listFiles(
  config: Pick<
    TypeInferConfig,
    'cwd' | 'allowed' | 'notallowed' | 'ignore' | 'include'
  >,
) {
...
function prim(t: string): InferredType {
  return t
}
...
class InferVisitor extends Visitor {
  ...
  override visitTsType(n: any): any {
    return n
  }
  override visitTsTypeAnnotation(n: any): any {
    return n
  }
  ...
  private inferExpr(expr: any): InferredType {
    ...
    switch (expr.type) {
      ...
      case 'ArrayExpression': {
        const elems = (expr as ArrayExpression).elements
          ?.map((e) => e?.expression && this.inferExpr(e.expression))
      ...
      case 'CallExpression': {
        ...
        if (
          ...
          (ce.callee.property as any).type === 'Identifier' &&
          (ce.callee.property as any).value === 'then'
        ) {
          return 'Promise<any>'
        }
        ...
        if (
          ...
          if (!this.functionsByName.has(name))
            this.functionsByName.set(name, {
              ...
              params: {},
              returns: new Set(),
              spanStart: (n as any).span?.start ?? 0,
            })
          ...
          for (const p of params) {
            ...
            if (pat?.type === 'Identifier')
              finfo.params[pat.value] =
                finfo.params[pat.value] || new Set<InferredType>()
            else if (pat?.pat?.type === 'Identifier')
              finfo.params[pat.pat.value] =
                finfo.params[pat.pat.value] || new Set<InferredType>()
          }
          if (
            init.type === 'ArrowFunctionExpression' &&
            init.body &&
  ...
  override visitBinaryExpression(n: BinaryExpression): any {
    ...
    if (be.operator === '+') {
      ...
      const leftIsTypeof =
        ...
        (l as UnaryExpression).operator === 'typeof'
      const rightIsString = r?.type === 'StringLiteral'
      const rightIsTypeof =
        r?.type === 'UnaryExpression' &&
        (r as UnaryExpression).operator === 'typeof'
    ...
    if (
      ...
      if (!left.computed && prop?.type === 'Identifier') {
        ...
        if (!this.varTypes.has(objName))
          this.varTypes.set(objName, {
            names: new Set([objName]),
            inferred: new Set(),
          })
  ...
  override visitCallExpression(n: CallExpression): any {
    ...
    if (ce.callee?.type === 'MemberExpression') {
      ...
      if (obj?.type === 'Identifier' && prop?.type === 'Identifier') {
        if (prop.value === 'push' && ce.arguments?.[0]) {
          this.recordVar(
            obj.value,
            arrayOf(this.inferExpr(ce.arguments[0].expression)),
          )
        }
        if (
          obj?.type === 'Identifier' &&
    ...
    if (ce.callee?.type === 'Identifier') {
      ...
      if (!this.functionsByName.has(fnName))
        this.functionsByName.set(fnName, {
          name: fnName,
          params: {},
          returns: new Set(),
          spanStart: 0,
      ...
      ce.arguments?.forEach((arg: any, idx: number) => {
        ...
        if (pname) {
          ...
          finfo.params[synthetic] =
            finfo.params[synthetic] || new Set<InferredType>()
...
export function inferTypesInFiles(config: TypeInferConfig) {
  ...
  for (const file of files) {
    ...
    const { globalVariables } = processUnusedAndGlobalVariables(program, {
      knownGlobals: config.knownGlobals,
    })
    for (const name of globalVariables.keys()) {
      const info = varTypes.get(name) ?? {
        names: new Set([name]),
        inferred: new Set<InferredType>(['any']),
      }
      if (!globalsAgg.has(name))
        globalsAgg.set(name, {
          names: new Set([name]),
          inferred: new Set<InferredType>(),
        })
    ...
    if (config.exportFunctions && config.exportFunctions.length > 0) {
      ...
      program.body.forEach((stmt: any) => {
        if (stmt.type === 'FunctionDeclaration')
          decls.push(stmt as FunctionDeclaration)
      })
      ...
      for (const fn of sorted) {
        ...
        updatedSource =
          updatedSource.slice(0, lineStart) +
          'export ' +
          updatedSource.slice(lineStart)
      }
    ...
    const funcNames = new Set(
      fileFuncs.filter((f) => f.name).map((f) => f.name as string),
    )
    for (const finfo of fileFuncs) {
      ...
      if (!isTS || genForTS) {
        ...
        const paramsText = paramNames
          .map(
            (pn) =>
              `${pn}: ${mergeTypes(finfo.params[pn] ?? new Set<InferredType>(['any']))}`,
          )
          .join(', ')
        const retType =
          mergeTypes(finfo.returns ?? new Set<InferredType>(['any'])) || 'any'
        fnLines.push(
          `${exportIt ? 'export ' : 'declare '}function ${name}(${paramsText}): ${retType};`,
        )
      }
    }
    for (const [vname, vinfo] of (varTypes as Map<string, VarInfo>).entries()) {
      if (funcNames.has(vname)) continue
      const vtype = vinfo.objectShape
        ? objectOf(vinfo.objectShape)
        : mergeTypes(vinfo.inferred)
    ...
    report.files[file] = {
      functions: (funcsByFile.get(file) || []).map((f) => ({
        name: f.name,
        params: Object.fromEntries(
          Object.entries(f.params).map(([k, v]) => [k, Array.from(v)]),
        ),
        returns: Array.from(f.returns),
      })),
      variables: Array.from(varTypes.entries()).map(([name, info]) => ({
        name,
        type: info.objectShape
          ? objectOf(info.objectShape)
          : mergeTypes(info.inferred),
      })),
    }
  }
  ...
  for (const [name, info] of globalsAgg.entries()) {
    const typeStr = info.objectShape
      ? objectOf(info.objectShape)
      : mergeTypes(info.inferred)
    ...
    for (const file of files) {
      ...
      program.body.forEach((stmt: any) => {
        if (stmt.type === 'FunctionDeclaration') {
          ...
          let hasTypes = false
          if (isTS) {
            const hasAnyParam = (stmt.params || []).some((p: any) =>
              p.pat?.typeAnnotation
                ? p.pat.typeAnnotation.typeAnnotation.type !== 'TsAnyKeyword'
                : false,
            )
          ...
          const params = fI
            ? Object.keys(fI.params).map(
                (pn) => `${pn}: ${mergeTypes(fI.params[pn]) || 'any'}`,
              )
            : []
      ...
      for (const fn of sorted) {
        const before = augmented.slice(
          Math.max(0, fn.start - program.span.start - 4),
          fn.start - program.span.start,
        )
        ...
        augmented =
          augmented.slice(0, insertPos) + jsdoc + augmented.slice(insertPos)
      }
...
export function inferTypesInSource(
  source: string,
  knownGlobals: string[] = [],
): {
  ...
  try {
    ...
    const { globalVariables } =
      require('./ident').processUnusedAndGlobalVariables(program, {
        knownGlobals,
      })
    const result: Record<string, InferredType> = {}
    for (const name of globalVariables.keys()) {
      const info = varTypes.get(name)
      const t = info?.objectShape
        ? objectOf(info.objectShape)
        : mergeTypes(info?.inferred ?? new Set<InferredType>())
      result[name] = t || 'any'
    }
