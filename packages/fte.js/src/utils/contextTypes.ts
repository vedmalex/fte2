...
function buildAnalysisCode(template: any): string {
  ...
  try {
    ...
    // blocks
    const blocks = template?.blocks ? Object.values(template.blocks) as any[] : []
    ...
    // slots
    const slots = template?.slots ? Object.values(template.slots) as any[] : []
...
export function generateContextTypes(template: any): GeneratedContextTypes {
  const name = (template?.name || 'Template').replace(/[^A-Za-z0-9_]/g, '_') + '_Context'
  const code = buildAnalysisCode(template)
  const fields = new Set<string>()
  const types: Record<string, string> = {}
  if (code && code.trim().length) {
    const inferred = inferTypesInSource(code, ['out', 'options', 'Array', 'Object']).variables
