import * as visitor from './typeinfer_swc'
...
describe('inferTypesFromFunction', () => {
  ...
  it('extracts all parameters of function', () => {
    ...
    expect(func14?.properties.size).toBe(3)
    expect(func14?.properties.get('a')).not.toBeUndefined()
    expect(func14?.properties.get('b')).not.toBeUndefined()
    expect(func14?.properties.get('c')).not.toBeUndefined()
  })
})
