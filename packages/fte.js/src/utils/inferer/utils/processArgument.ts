...
export function processArgument(
  ...
  if (t.isIdentifier(node)) {
    ...
    const parameter = extractName(node.parameter, anonymous)
    const param = createInfo(
      actualAgruments,
      parameter,
      parameter,
    ...
    const parameter = extractName(node.argument, anonymous)
    const param = createInfo(
      actualAgruments,
      parameter,
      parameter,
    ...
    if (t.isObjectPattern(node)) {
      ...
      node.properties.forEach((p) => {
        const parameter = extractName(p, anonymous)
        const param = createInfo(
          actualAgruments,
          parameter,
          parameter,
      ...
      const parameter = extractName(node.left, anonymous)
      const param = createInfo(
        actualAgruments,
        parameter,
        parameter,
