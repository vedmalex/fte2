...
var MainTemplate_njs_default = {
  ...
  script: function(context, _content, partial, slot, options) {
    ...
    var out = [];
    var directives2 = context.directives;
    out.push("{\n");
    if (directives2.chunks)
      out.push("\nchunks: " + JSON.stringify(directives2.chunks) + ",\n");
    out.push("\n");
    ...
    if (directives2.requireAs.length > 0) {
      out.push("\n    this.aliases={}\n");
      var rq;
      for (var i2 = 0, len = directives2.requireAs.length; i2 < len; i2++) {
        rq = directives2.requireAs[i2];
...
function prepareCode(src2) {
  const result = swc.transformSync(src2, {
    jsc: {
      minify: {
        compress: {
          dead_code: true,
